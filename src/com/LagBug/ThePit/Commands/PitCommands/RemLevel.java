...
public class RemLevel {
	
    public static boolean onCommand(CommandSender sender, Command cmd, String label, String[] args, Main main) {
    	
    	Player player = (Player) sender; 
    	
		if (!player.hasPermission("pit.admin.remlevel") || !player.hasPermission("pit.admin.*") || !player.hasPermission("pit.*")) {
			player.sendMessage(main.getMessage("general.no-permission"));
			return false;
		}
		
		if (args.length <= 2) {
			player.sendMessage(main.getMessage("commands.right-usage").replace("%usage%", "/pit remlevel <player> <amount>"));
			...
			if (leveledupPlayer == null) {
				...
				if (level < 0 || level > 120) {
					...
					player.sendMessage(main.getMessage("commands.level.remove").replace("%player%", leveledupPlayer.getName()).replace("%amount%", Integer.toString(level)));
