...
public class RemGold {
    ...
    public static boolean onCommand(CommandSender sender, Command cmd, String label, String[] args, Main main) {
		...
		if (!player.hasPermission("pit.admin.remgold") || !player.hasPermission("pit.admin.*") || !player.hasPermission("pit.*")) {
			player.sendMessage(main.getMessage("general.no-permissions"));
			return false;
		}
		
		if (args.length <= 2) {
			player.sendMessage(main.getMessage("commands.right-usage").replace("%usage%", "/pit remgold <player> <amount>"));
			...
			if (goldPlayer == null) {
				...
				player.sendMessage(main.getMessage("commands.gold.rem"));
				pm.removeGold(newGold);
			}
		}	
